...
namespace Habr.DataAccess.Services
{
    public class CommentService : ICommentService
    {
        ...
        public void CreateCommentAnswer(int userId, string text, int parentId, int postId)
        {
            ...
            context.Comments.Add(new Comment { User = user, Text = text, Parent = parent, Post = post });
            context.SaveChanges();
        }
        ...
        {
            using var context = new DataContext();
            return context.Comments.AsNoTracking().ToList();
        }

        public IEnumerable<Comment> GetCommentsByPost(int postId)
        {
            using var context = new DataContext();
            return context.Comments.Where(c => c.PostId == postId ).Include(c => c.User).Include(c => c.SubComments).AsNoTracking().ToList();
        }

        public IEnumerable<Comment> GetCommentsByUser(int userId)
        {
            using var context = new DataContext();
            return context.Comments.Where(c => c.UserId == userId).Include(c => c.SubComments).AsNoTracking().ToList(); 
        }
    }
}
